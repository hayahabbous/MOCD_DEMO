...
//            "sidemenu_27"
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        sideMenuCell.lblSidemenuTitle.text = arrSidemenuList[indexPath.row].uppercased()
    ...
    @IBAction func btnSideMenuAction(_ sender: UIButton) {
        ...
        if let m = mm_drawerController {
            if (m.leftDrawerViewController != nil) {
                m.toggle(MMDrawerSide.left, animated: true, completion: nil)
            }
        }
    }
