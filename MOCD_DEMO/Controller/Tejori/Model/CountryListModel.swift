...
class CountryListModel: NSObject {
    ...
    func GetAllCountryList() -> [CountryListModel] {
        ...
        let aMutArrCountrylist : [[String : AnyObject]] = DatabaseEdkhar.sharedInstance.selectAllfromTable(query: aStrGetAllQuery) { (status) in
        ...
        if aMutArrCountrylist.count > 0 {
            for aDictCountryInfo in aMutArrCountrylist {
                ...
                objCountryModel.identifier = NSInteger(aStrCountryID!)
