...
class AddUserVCT: UIViewController {
    ...
    var objUserInfoModel : userInfoModelT?
    ...
    @objc func doneAction() -> Void {
        ...
        if NSDate().compare((self.pickerDateTime?.date)!) == ComparisonResult.orderedDescending{
            //Do Something...
            self.showAlertController(aStrTitle: "Alert", aStrMsg: "Please shedule reminder for future date", aTag: 0)
        }
        else{
    ...
    @IBAction func btnAddUserDoneAction(_ sender: UIBarButtonItem) {
        ...
        if (tfUsername.text?.isEmpty)! {
            self.showAlertController(aStrTitle: "Alert", aStrMsg: "Enter Username", aTag: 0)
        }
        else if (tfUserdob.text?.isEmpty)! {
            self.showAlertController(aStrTitle: "Alert", aStrMsg: "Enter Date of birth", aTag: 0)
        }
        else{
            ...
            if self.objUserInfoModel != nil {
                ...
                let aInsertStatus : Bool = userInfoManagerT().UpdateUserInfoDetail(objUserInfo: self.objUserInfoModel!)
                ...
                let aInsertStatus : Bool = userInfoManagerT().InsertUserInfoDetail(objUserInfo: objUserModel)
                if aInsertStatus == true {
...
//            dateFormatter.timeZone = NSTimeZone(abbreviation: "UTC") as TimeZone!
            ...
            self.eventStore.requestAccess(to: EKEntityType.reminder, completion: { (granted : Bool, error : Error?) in
                if granted == true{
                    ...
                    self.showAlertController(aStrTitle: "Alert", aStrMsg: "The app is not permitted to access reminders, make sure to grant permission in the settings and try again!", aTag: 0)
                }
            })
        }
    }
    ...
    func showAlertController(aStrTitle : String , aStrMsg : String , aTag : Int) -> Void {
        ...
        let aAlertAction = UIAlertAction.init(title: "OK", style: UIAlertAction.Style.default) { (action) in
            if aTag == 1{
...
extension AddUserVC: UNUserNotificationCenterDelegate {
